/**
 * 🛡️ Nexus Mind Vault — Complete W3C Web Crypto API Engine
 * Standards Compliance & Cryptographic Verification:
 * 1. Key Derivation: PBKDF2-HMAC-SHA-256 (100,000 iterations, 128-bit CSPRNG Salt)
...
// ============================================================================
// 2. PBKDF2-SHA-256 KEY DERIVATION (100,000 ITERATIONS)
// ============================================================================
...
export async function deriveKeyFromPassphrase(
  ...
  return await crypto.subtle.deriveKey(
    ...
    true, // extractable for wrapKey export backups
...
export async function verifySecretPassphrase(
  ...
  storedCredentials: { salt?: string; secretVerifier?: string; secret?: string }
): Promise<boolean> {
  if (!storedCredentials) return false;

  // 1. Modern Zero-Knowledge Verifier Hash Check
  if (storedCredentials.salt && storedCredentials.secretVerifier) {
  ...
  // 2. Legacy Plaintext Migration Fallback
  if (storedCredentials.secret) {
    return enteredSecret.trim() === storedCredentials.secret.trim();
  }
...
export async function verifyPinCode(
  enteredPin: string,
  storedCredentials: { salt?: string; pinHash?: string; pin?: string }
): Promise<boolean> {
  if (!storedCredentials) return false;

  // 1. Modern Zero-Knowledge PIN Hash Check
  if (storedCredentials.salt && storedCredentials.pinHash) {
    const saltBytes = base64ToBuffer(storedCredentials.salt);
  ...
  // 2. Legacy Plaintext Migration Fallback
  if (storedCredentials.pin) {
    return enteredPin.trim() === storedCredentials.pin.trim();
  }
...
export async function wrapCryptoKey(
  ...
  const iv = generateRandomIV(12);
  const wrappingKey = await deriveKeyFromPassphrase(exportPassphrase, salt, 100000);
...
/**
 * 🔒 Web Crypto API Asynchronous SHA-256 Digest (64-character lowercase hex)
 */
...
/**
 * 🔒 NIST FIPS 180-4 Compliant Synchronous Cryptographic SHA-256 Hash
...
export function verifyTimeCapsuleIntegrity(capsule: any): { isValid: boolean; calculatedHash: string; storedHash: string } {
  ...
  // Handle legacy demo hashes or exact hash match
  const isValid = calculatedHash === storedHash || storedHash.startsWith('sha256_');
